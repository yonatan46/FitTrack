  ...
  return (
    <main className="app-shell">
      ...
      <section className="main-content" id="dashboard">
        ...
        {profileModalOpen && <section className="profile-popover" role="dialog" aria-modal="false" aria-labelledby="profile-modal-title"><button className="modal-close" onClick={() => setProfileModalOpen(false)} aria-label="Close profile"><X size={18} /></button><div className="modal-avatar">{(profile?.display_name || "A").slice(0, 2).toUpperCase()}</div><p className="eyebrow">Your account</p><h2 id="profile-modal-title">{profile?.display_name || "Athlete"}</h2><p className="modal-copy">Manage your training preferences and account.</p><Link className="primary-button modal-settings" href="/settings" onClick={() => setProfileModalOpen(false)}><Settings2 size={16} />Open settings</Link><button className="modal-signout" onClick={() => signOut()}>Sign out</button></section>}
